...
class _PermissionRequestScreenState extends State<PermissionRequestScreen> with SingleTickerProviderStateMixin {
  ...
  int _rowsPerPage = 10;
  int _currentPage = 0;
  List<dynamic> _dateFilteredHistory = [];
    ...
    setState(() {
      ...
      if (query.isEmpty) {
        ...
        _filteredHistory = _dateFilteredHistory.where((request) => 
          (request['TicketNo']?.toString().toLowerCase().contains(query) ?? false) ||
          (request['EmpName']?.toString().toLowerCase().contains(query) ?? false)
        ).toList();
      }
      _currentPage = 0;
    });
  ...
  Widget _buildHistoryCards() {
    ...
    List<dynamic> displayedItems = _filteredHistory.sublist(start, end);
